  ...
  const handleStart = async () => {
    ...
    if (method === "import" && sourceType === "youtube") {
      ...
      try {
        ...
        if (urls.length === 1) {
          sessionStorage.setItem("study-pdf-url", urls[0]);
        } else {
          sessionStorage.setItem("study-pdf-urls", JSON.stringify(urls));
        }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
        ...
        {method === "import" && (
          <div className="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-300">
            {/* Back button */}
            <button
              onClick={() => { setMethod(null); setYoutubeUrl(""); setPdfFiles([]); }}
            ...
            <div className="flex bg-gray-100 rounded-xl p-1 mb-6">
              {(["youtube", "pdf"] as SourceType[]).map((src) => (
                <button
                  key={src}
                  onClick={() => { setSourceType(src); setYoutubeUrl(""); setPdfFiles([]); }}
                  ...
                  {pdfFiles.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400 mt-1">{t("study.pdfUpload.hintMulti")}</p>
                      </div>
                    </div>
                  )}
                </div>
